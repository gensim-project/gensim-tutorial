...
\smash{
	\begin{tikzpicture}[fill opacity=0.3]
		\draw[red,thick] (7.45, 0.5) rectangle (8.8, 1);
		\draw[draw=none] (0,0) rectangle (0, 1);
	\end{tikzpicture}%
}
}
...
\only<4>{The Memory Model - How should underlying memory be stored?}
\only<5>{The target binary - which binary should be loaded for simulation?}
\end{minipage}
